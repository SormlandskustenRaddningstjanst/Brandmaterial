...
const views={
 ...
 consumables:{title:"Förbrukningsartiklar",help:"Saldo, beställningsnivå och leverantör per station. Artikel-ID används i hyllans QR-kod.",columns:["Artikel-ID","Artikel","Kategori","Station","Saldo","Beställ vid","Önskat lager","Enhet","Leverantör","Artikelnummer","Beställningslänk","Kommentar","Aktiv"],headers:["Artikel-ID","Artikel","Kategori","Station","Saldo","Beställ vid","Önskat lager","Enhet","Leverantör","Artikelnummer","Beställningslänk","Kommentar","Aktiv"],endpoint:null}
};
...
async function createConsumable(){
 ...
 btn.disabled=true;m.className="message active";m.textContent="Skapar…";
 try{
  const result=await postJson("/consumable",{article,category,stationId,balance,reorderAt,target,unit,supplier,supplierArticleNumber,orderUrl,comment});
  ...
  ["newConsumableName","newConsumableCategory","newConsumableSupplier","newConsumableSupplierNo","newConsumableOrderUrl","newConsumableComment"].forEach(id=>$(id).value="");
  $("newConsumableUnit").value="st";$("newConsumableBalance").value="0";$("newConsumableReorder").value="0";$("newConsumableTarget").value="0";
