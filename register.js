...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/register-data") {
        ...
        const material = materialData.results.map(row => {
          ...
          return {
            ...
            exerciseMaterial: row["Övningsmaterial"] === true,
            station: stationId ? (stationNameById.get(Number(stationId)) || "") : "",
      ...
      // POST /consumable/bulk-update – sparar endast ändrade registerrader i ett batch-anrop.
      // Saldo och Station ändras avsiktligt inte här. Saldo ska gå via lagerhändelser/QR.
      if (request.method === "POST" && url.pathname === "/consumable/bulk-update") {
        if (!isAllowedBrowserOrigin(request)) return json({error:"Otillåten origin"},403,corsHeaders);
        const body = await readJson(request);
        const rows = Array.isArray(body?.rows) ? body.rows : [];
        if (!rows.length) return json({success:true,updatedCount:0},200,corsHeaders);
        if (rows.length > 200) return json({error:"Högst 200 rader kan sparas åt gången."},400,corsHeaders);

        const all = await baserowRequest(env, `/database/rows/table/${TABLES.consumables}/?user_field_names=true&size=200`, {method:"GET"});
        const byArticleId = new Map(all.results.map(r => [normalizeConsumableId(r["Artikel-ID"]), r]));
        const items = [];
        const seen = new Set();
        const numOrNull = v => String(v ?? "").trim() === "" ? null : Number(v);
        for (const item of rows) {
          const articleId = normalizeConsumableId(item?.articleId);
          if (!isValidConsumableId(articleId)) return json({error:`Ogiltigt Artikel-ID: ${articleId || "saknas"}`},400,corsHeaders);
          if (seen.has(articleId)) return json({error:`Dubblett i sparningen: ${articleId}`},400,corsHeaders);
          seen.add(articleId);
          const existing = byArticleId.get(articleId);
          if (!existing) return json({error:`Artikeln hittades inte: ${articleId}`},404,corsHeaders);
          const reorderAt=numOrNull(item.reorderAt), target=numOrNull(item.target), packageSize=numOrNull(item.packageSize), minimumOrderQuantity=numOrNull(item.minimumOrderQuantity);
          for (const [label,value] of [["Beställ vid",reorderAt],["Önskat lager",target],["Förpackningsstorlek",packageSize],["Minsta beställningsantal",minimumOrderQuantity]]) {
            if (value !== null && (!Number.isInteger(value) || value < 0)) return json({error:`${articleId}: ${label} måste vara ett heltal 0 eller högre.`},400,corsHeaders);
          }
          const article=String(item.article||"").trim();
          if(!article) return json({error:`${articleId}: Artikel måste anges.`},400,corsHeaders);
          items.push({
            id: existing.id,
            "Artikel": article,
            "Kategori": String(item.category||"").trim(),
            "Beställ vid": reorderAt,
            "Önskat lager": target,
            "Enhet": String(item.unit||"st").trim() || "st",
            "Leverantör": String(item.supplier||"").trim(),
            "Artikelnummer": String(item.supplierArticleNumber||"").trim(),
            "Kontaktperson": String(item.contactPerson||"").trim(),
            "Telefon": String(item.phone||"").trim(),
            "E-post": String(item.email||"").trim(),
            "Kundnummer": String(item.customerNumber||"").trim(),
            "Avtalsnummer": String(item.agreementNumber||"").trim(),
            "Förpackningsstorlek": packageSize,
            "Minsta beställningsantal": minimumOrderQuantity,
            "Beställningslänk": String(item.orderUrl||"").trim(),
            "Beställningskommentar": String(item.orderComment||"").trim(),
            "Användningsområde": String(item.usageArea||"").trim(),
            "Kommentar": String(item.comment||"").trim(),
            "Aktiv": item.active === true
          });
        }
        const updated = await baserowRequest(env, `/database/rows/table/${TABLES.consumables}/batch/?user_field_names=true`, {method:"PATCH",body:{items}});
        return json({success:true,updatedCount:Array.isArray(updated?.items)?updated.items.length:items.length},200,corsHeaders);
      }

      // POST /material/bulk-update – sparar flera redigerade Brandmaterial-rader i ett anrop.
      // Material-ID används som låst identitet och får inte ändras här.
      if (request.method === "POST" && url.pathname === "/material/bulk-update") {
        if (!isAllowedBrowserOrigin(request)) return json({error:"Otillåten origin"},403,corsHeaders);
        const body=await readJson(request);
        const rows=Array.isArray(body?.rows)?body.rows:[];
        if(!rows.length) return json({error:"Inga ändringar att spara."},400,corsHeaders);
        if(rows.length>500) return json({error:"Max 500 ändrade rader per sparning."},400,corsHeaders);

        const materialData=await baserowRequest(env,`/database/rows/table/${TABLES.material}/?user_field_names=true&size=200`,{method:"GET"});
        const byId=new Map(materialData.results.map(r=>[normalizeMaterialId(r["Material-ID"]),r]));
        const errors=[];
        const prepared=[];
        const seen=new Set();
        for(let i=0;i<rows.length;i++){
          const src=rows[i]||{};
          const materialId=normalizeMaterialId(src.materialId);
          if(!isValidMaterialId(materialId)){errors.push(`Rad ${i+1}: ogiltigt Material-ID.`);continue;}
          if(seen.has(materialId)){errors.push(`${materialId} förekommer flera gånger.`);continue;}
          seen.add(materialId);
          const existing=byId.get(materialId);
          if(!existing){errors.push(`${materialId} hittades inte.`);continue;}
          const patch={};
          if(Object.prototype.hasOwnProperty.call(src,"material")) patch["Material"]=String(src.material??"").trim();
          if(Object.prototype.hasOwnProperty.call(src,"category")) patch["Kategori"]=String(src.category??"").trim();
          if(Object.prototype.hasOwnProperty.call(src,"exerciseMaterial")){
            patch["Övningsmaterial"]=src.exerciseMaterial===true;
          }
          if(Object.prototype.hasOwnProperty.call(src,"comment")) patch["Kommentar"]=String(src.comment??"");
          if(Object.prototype.hasOwnProperty.call(src,"active")) patch["Aktiv"]=src.active===true;
          if(!String(patch["Material"]??existing["Material"]??"").trim()){errors.push(`${materialId}: Material får inte vara tomt.`);continue;}
          prepared.push({materialId,rowId:existing.id,patch});
        }
        if(errors.length) return json({success:false,errors},400,corsHeaders);
        let updatedCount=0;
        for(const item of prepared){
          if(!Object.keys(item.patch).length) continue;
          await baserowRequest(env,`/database/rows/table/${TABLES.material}/${item.rowId}/?user_field_names=true`,{method:"PATCH",body:item.patch});
          updatedCount++;
        }
        return json({success:true,updatedCount},200,corsHeaders);
      }

      // POST /material/exercise – anger om ett individuellt SKRTJ-ID även får användas som övningsmaterial.
      // Alla poster är alltid Brandmaterial; Övningsmaterial är endast en Ja/Nej-markering.
      if (request.method === "POST" && url.pathname === "/material/exercise") {
        if (!isAllowedBrowserOrigin(request)) return json({error:"Otillåten origin"},403,corsHeaders);
        const body=await readJson(request);
        const materialId=normalizeMaterialId(body?.materialId);
        const exerciseMaterial=body?.exerciseMaterial===true;
        if(!isValidMaterialId(materialId)) return json({error:"Ogiltigt Material-ID"},400,corsHeaders);
        const material=await findMaterialById(env,materialId);
        if(!material) return json({error:"Materialet hittades inte"},404,corsHeaders);
        const updated=await baserowRequest(env,`/database/rows/table/${TABLES.material}/${material.id}/?user_field_names=true`,{method:"PATCH",body:{"Övningsmaterial":exerciseMaterial}});
        return json({success:true,materialId,exerciseMaterial,rowId:updated.id},200,corsHeaders);
      }
      ...
      if (request.method === "POST" && url.pathname === "/material/import") {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          if (has("Övningsmaterial")) {
            const v = String(source["Övningsmaterial"]).trim().toLowerCase();
            if (!["true","false","sant","falskt","ja","nej","1","0"].includes(v) && typeof source["Övningsmaterial"] !== "boolean") {
              errors.push(`Rad ${excelRow}: Övningsmaterial måste vara Ja eller Nej.`);
            } else {
              patch["Övningsmaterial"] = source["Övningsmaterial"] === true || ["true","sant","ja","1"].includes(v);
            }
          }
            ...
            await baserowRequest(
              ...
              `/database/rows/table/${TABLES.material}/?user_field_names=true`,
              {
                ...
                body:{
                  ...
                  "Övningsmaterial": item.patch["Övningsmaterial"] ?? false,
                  ...item.patch,
                }
              }
            );
      ...
      if (request.method === "GET" && url.pathname === "/exercise-rules") {
        ...
        const eligibleRows=materialData.results.filter(r=>r["Aktiv"]===true && r["Övningsmaterial"]===true);
        const materials=[...new Set(eligibleRows.map(r=>String(r["Material"]||"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"sv"));
        const availableByMaterial=Object.fromEntries(materials.map(name=>[name,eligibleRows.filter(r=>sameMaterialName(r["Material"],name)).length]));
        ...
        return json({materials,rules,availableByMaterial},200,corsHeaders);
      }
      ...
      if (request.method === "GET" && url.pathname === "/orders") {
        ...
        const requestedStationIds=String(url.searchParams.get("stationIds")||"").split(",").map(Number).filter(Number.isInteger);
        const nykopingView=url.searchParams.get("nykoping")==="1";
        let orders=[...normal,...exerciseMap.values()];
        if(!nykopingView&&requestedStationIds.length){
          orders=orders.filter(o=>requestedStationIds.includes(Number(o.stationId)));
        }
      ...
      if (request.method === "POST" && url.pathname === "/orders") {
        ...
        const [levels,exerciseMaterialData]=await Promise.all([
          baserowRequest(env,`/database/rows/table/${TABLES.stockLevels}/?user_field_names=true&size=200`,{method:"GET"}),
          baserowRequest(env,`/database/rows/table/${TABLES.material}/?user_field_names=true&size=200`,{method:"GET"})
        ]);
        const eligibleExerciseRows=exerciseMaterialData.results.filter(r=>r["Aktiv"]===true && r["Övningsmaterial"]===true);
        ...
        for(const x of lines){const material=String(x?.material||"").trim(); const quantity=Number(x?.quantity); const k=material.toLocaleLowerCase("sv-SE"); if(!material||seen.has(k)) return json({error:"Varje materialtyp får bara finnas en gång."},400,corsHeaders); seen.add(k); const max=rules.get(k); if(!max) return json({error:`${material} är inte beställningsbar för övning.`},409,corsHeaders); const available=eligibleExerciseRows.filter(r=>sameMaterialName(r["Material"],material)).length; const allowed=Math.min(max,available); if(allowed<1) return json({error:`${material} saknar tillgängliga SKRTJ-ID med Övningsmaterial = Ja.`},409,corsHeaders); if(!Number.isInteger(quantity)||quantity<1||quantity>allowed) return json({error:`${material}: antal måste vara 1–${allowed}.`},409,corsHeaders); clean.push({material,quantity});}
